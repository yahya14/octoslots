...
namespace Octoslots
{
    ...
    public class TCPGecko
    {
        ...
        public void Dump(UInt32 startdump, UInt32 enddump, Stream[] saveStream)
        {
            ...
            if (Octoslots.ValidMemory.rangeCheckId(startdump) != Octoslots.ValidMemory.rangeCheckId(enddump))
            {
                enddump = Octoslots.ValidMemory.ValidAreas[Octoslots.ValidMemory.rangeCheckId(startdump)].high;
            }

            if (!Octoslots.ValidMemory.validAddress(startdump)) return;
        ...
        */
        ...
        {
            if (!Octoslots.ValidMemory.validAddress(address))
            {
                return 0;
            }
        ...
        public uint[] peek(UInt32 address, uint length)
        {
            //round the length to the nearest 4
            uint rlength = (uint)Math.Round(length / 4.0) * 4;

            if (!global::Octoslots.ValidMemory.validAddress(address))
            {
                return new uint[rlength / 4];
            }
            ...
            {
                //dump data
                Dump(paddress, paddress + rlength, stream);

                //define byte array
                stream.Seek(0, SeekOrigin.Begin);
                byte[] buffer = stream.GetBuffer();

                //create uint array for the dumped addresses
                uint[] uintArray = new uint[rlength / 4];
                for (int i = 0; i < rlength / 4; i++)
                {
                    uintArray[i] = ByteSwap.Swap(BitConverter.ToUInt32(buffer, i * 0x4));
                }

                return uintArray;
                /*
                 * To call this method, create a new array as follows:
                 *     uint[] something = new uint[0]
                 *     something = gecko.peek(0x10000000, 8);
                 * By this example, in return we can get an array of 2 values of the 2 addresses in something[]. This incl. the values for 0x10000000 and 0x10000004
                 * Flexible huh?
                 */
            }
            finally
